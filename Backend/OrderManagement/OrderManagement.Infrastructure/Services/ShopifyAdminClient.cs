...
public class ShopifyAdminClient
{
    ...
    """;
        ...
        var graphQlResponse =
            JsonSerializer.Deserialize<ShopifyGraphQlResponse<ShopifyWebhookCreateData>>(responseText);
        ...
        var payload = graphQlResponse.Data?.WebhookSubscriptionCreate;

        if (payload is null)
        {
            throw new InvalidOperationException("Shopify did not return webhook subscription data.");
        }

        if (payload.UserErrors.Count > 0)
        {
            var errors = string.Join(" | ", payload.UserErrors.Select(x => x.Message));
            throw new InvalidOperationException($"Shopify webhook error: {errors}");
        }

        return payload.WebhookSubscription?.Id
            ?? throw new InvalidOperationException("Shopify did not return a webhook subscription ID.");
    }
